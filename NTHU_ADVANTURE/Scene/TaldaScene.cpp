...
void TaldaScene::Initialize() {
    ...
    // 開始背景音樂
    bgmId = AudioHelper::PlayBGM("telda.ogg");
...
bool TaldaScene::IsPlayerNearBuilding(Player* player, const BuildingZone& zone) {
    // 將建築物格子坐標轉為像素坐標
...
void TaldaScene::ShowEnterPrompt(const std::string& buildingName, int zoneX, int zoneY) {
...
void TaldaScene::AddBuildingZone(int x, int y, int width, int height, const std::string& buildingName) {
    // 設置建築物的範圍區域並儲存
