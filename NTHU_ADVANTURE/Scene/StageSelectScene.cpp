...
void StageSelectScene::Initialize() {
    ...
    btn->SetOnClickCallback(std::bind(&StageSelectScene::PlayOnClick, this, 2));//PlayOnClick 接受一個 int stage 參數
    AddNewControlObject(btn);
    ...
    btn->SetOnClickCallback(std::bind(&StageSelectScene::ScoreboardOnClick, this));//ScoreboardOnClick不接受任何參數
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Scoreboard", "pirulen.ttf", 40, halfW, halfH / 2 + 300, 0, 0, 0, 255, 0.5, 0.5));
    //BackOnClick 接受 int stage
